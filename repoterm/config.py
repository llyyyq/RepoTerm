...
REPOTERM_DIR = Path.home() / ".repoterm"
REPOTERM_SETTINGS_PATH = REPOTERM_DIR / "settings.json"
...
def load_effective_settings(
    ...
    *,
    ...
    global_mcp = read_mcp_config_file(REPOTERM_MCP_PATH)
    ...
    if trust_project_mcp:
        ...
        import logging
        logging.getLogger("repoterm.config").warning(
            "Project .mcp.json found at %s but NOT loaded (security: use "
            "--trust-project-mcp or REPOTERM_TRUST_PROJECT_MCP=1).", pmp,
        )

    repoterm_settings = read_settings_file(REPOTERM_SETTINGS_PATH)
...
def save_repoterm_settings(updates: dict[str, Any]) -> None:
    REPOTERM_DIR.mkdir(parents=True, exist_ok=True)
    existing = read_settings_file(REPOTERM_SETTINGS_PATH)
    next_settings = merge_settings(existing, updates)
    REPOTERM_SETTINGS_PATH.write_text(
...
def load_runtime_config(
    ...
    *,
    ...
    if trust_project_mcp is None:
        trust_project_mcp = os.environ.get("REPOTERM_TRUST_PROJECT_MCP", "").strip().lower() in (
            "1", "true", "yes", "on",
        )
    ...
    raw_max_output_tokens = (
        os.environ.get("REPOTERM_MAX_OUTPUT_TOKENS")
        or effective.get("maxOutputTokens")
        or env.get("REPOTERM_MAX_OUTPUT_TOKENS")
    )
    ...
    if not model:
        raise RuntimeError("No model configured. Set ~/.repoterm/settings.json or ANTHROPIC_MODEL.")
    if not has_auth:
        raise RuntimeError(
    ...
    # --- User profile paths ---
    global_user_profile = REPOTERM_USER_PROFILE_PATH
    proj_user_profile = project_user_profile_path(cwd)
    global_managed_policy = REPOTERM_MANAGED_POLICY_PATH
    proj_managed_policy = project_managed_policy_path(cwd)
    global_extensions = REPOTERM_EXTENSIONS_DIR
    ...
    response_language = (
        str(env.get("REPOTERM_LANGUAGE", "")).strip()
        or user_preferences.get("language", "")
    )
    response_verbosity = (
        str(env.get("REPOTERM_VERBOSITY", "")).strip()
        or user_preferences.get("verbosity", "")
    )
    fallback_models = _coerce_model_list(
        os.environ.get("REPOTERM_MODEL_FALLBACKS", "")
    ...
    return {
        ...
        "runtimeProfile": str(
            os.environ.get("REPOTERM_RUNTIME_PROFILE")
            or effective.get("runtimeProfile", "")
            or "single"
        ).strip().lower(),
        "toolProfile": str(
            os.environ.get("REPOTERM_TOOL_PROFILE")
            or effective.get("toolProfile", "")
            or "core"
        ).strip().lower(),
        "sourceSummary": f"config: {REPOTERM_SETTINGS_PATH} > {CLAUDE_SETTINGS_PATH} > process.env",
    }
...
def get_mcp_config_path(scope: str, cwd: str | Path | None = None) -> Path:
    return project_mcp_path(cwd) if scope == "project" else REPOTERM_MCP_PATH
...
def validate_config(cwd: str | Path | None = None) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        if "No model configured" in error_msg:
            suggestion = _suggest_model_name(os.environ.get("REPOTERM_MODEL", ""))
            help_msg = (
                f"Error: {error_msg}\n\n"
                "How to fix:\n"
                "  1. Set model name: export ANTHROPIC_MODEL=claude-sonnet-4-20250514\n"
                "  2. Or edit ~/.repoterm/settings.json:\n"
            ...
            help_msg = (
                ...
                "  5. Or edit ~/.repoterm/settings.json:\n"
