"""Interactive installer for RepoTerm.
...
def _install_launcher_script() -> str | None:
    ...
    if sys.platform == "win32":
        # Windows: Use ~/.repoterm/bin with .bat script
        target_bin_dir = REPOTERM_DIR / "bin"
        launcher_path = target_bin_dir / "repoterm.bat"
        python_exe = sys.executable.replace("/", "\\")
        launcher_script = "\r\n".join([
            "@echo off",
            "REM RepoTerm Launcher for Windows",
            f'"{python_exe}" -m repoterm.main %*',
            "",
        ])
        launcher_command = "repoterm.bat"
    elif sys.platform == "darwin":
        # macOS: Use ~/.local/bin with bash script (also works with zsh)
        target_bin_dir = home / ".local" / "bin"
        launcher_path = target_bin_dir / "repoterm"
        python_exe = sys.executable
        launcher_script = "\n".join([
            "#!/usr/bin/env bash",
            "# RepoTerm Launcher for macOS",
            "# Works with bash, zsh, and other shells",
            "set -euo pipefail",
            f'exec "{python_exe}" -m repoterm.main "$@"',
            "",
        ])
        launcher_command = "repoterm"
    else:
    ...
    try:
        ...
        print(f"\n⚠️  无法安装启动器脚本: {e}")
        print("你可以手动创建启动器脚本来调用 repoterm。")
        return None
...
def main() -> None:
    ...
    print("=" * 60)
    print()
    print(f"配置会写入: {REPOTERM_SETTINGS_PATH}")
    ...
    try:
        ...
        print(f"✅ 配置已保存到: {REPOTERM_SETTINGS_PATH}")
    except OSError as e:
        print(f"\n❌ 保存配置失败: {e}")
        sys.exit(1)
    ...
    print("=" * 60)
    print()
    print("📁 配置文件:", REPOTERM_SETTINGS_PATH)
    if launcher_result:
    ...
    print()
    print("  Windows:")
    print("    repoterm.bat               (如果已添加 PATH)")
    print("    python -m repoterm.main    (通用方式)")
    print()
    print("  macOS:")
    print("    repoterm                (如果已添加 PATH)")
    print("    python3 -m repoterm.main   (通用方式)")
    print()
    print("  Linux:")
    print("    repoterm                (如果已添加 PATH)")
    print("    python3 -m repoterm.main   (通用方式)")
