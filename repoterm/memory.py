...
- User memory (~/.repoterm/memory/) - cross-project, persistent
- Project memory (.repoterm-memory/) - shared across sessions, can be versioned
- Local memory (.repoterm-memory-local/) - project-specific, not checked in
...
class MemoryScope(str, Enum):
    """Memory scope levels."""
    USER = "user"       # Cross-project, ~/.repoterm/memory/
    PROJECT = "project" # Project-shared, .repoterm-memory/
    LOCAL = "local"     # Project-local, .repoterm-memory-local/
...
class MemoryPaths:
    ...
    def for_workspace(cls, workspace: str) -> "MemoryPaths":
        ...
        return cls(
            user_memory=REPOTERM_DIR / "memory",
            project_memory=workspace_path / ".repoterm-memory",
            local_memory=workspace_path / ".repoterm-memory-local",
        )
