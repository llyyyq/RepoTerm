...
from repoterm.state import add_cost, record_api_error, update_context_usage
from repoterm.types import AgentStep, StepDiagnostics

if TYPE_CHECKING:
    from repoterm.state import Store, AppState
...
def _get_retry_limit() -> int:
    try:
        value = int(float(os.environ.get("REPOTERM_MAX_RETRIES", DEFAULT_MAX_RETRIES)))
    except ValueError:
...
class AnthropicModelAdapter:
    ...
    def next(self, messages: list[dict[str, Any]], on_stream_chunk: Callable[[str], None] | None = None, on_thinking_delta: Callable[[str], None] | None = None, store: Store[AppState] | None = None) -> AgentStep:
        ...
        for attempt in range(max_retries + 1):
            try:
                timeout = int(os.environ.get("REPOTERM_MODEL_TIMEOUT", "60"))
