...
from repoterm.config import REPOTERM_PERMISSIONS_PATH

# Auto mode integration
from repoterm.auto_mode import AutoModeChecker, PermissionMode, get_mode_state
from repoterm.logging_config import log_permission_check
...
def _read_permission_store() -> dict[str, Any]:
    if not REPOTERM_PERMISSIONS_PATH.exists():
        return {}
    try:
        data = json.loads(REPOTERM_PERMISSIONS_PATH.read_text(encoding="utf-8"))
...
def _write_permission_store(store: dict[str, Any]) -> None:
    ...
    REPOTERM_PERMISSIONS_PATH.parent.mkdir(parents=True, exist_ok=True)
    
    # 写入临时文件
    fd, tmp_path = tempfile.mkstemp(
        dir=REPOTERM_PERMISSIONS_PATH.parent,
        suffix=".tmp"
    )
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            ...
            f.write('\n')
        # 原子替换
        os.replace(tmp_path, REPOTERM_PERMISSIONS_PATH)
    except Exception:
        # 清理临时文件
        try:
            os.unlink(tmp_path)
...
class PermissionManager:
    ...
    def ensure_path_access(self, target_path: str, intent: str) -> None:
        ...
        if self.prompt is None:
            ...
            raise RuntimeError(
                f"Path {normalized_target} is outside cwd {self.workspace_root}. Start repoterm in TTY mode to approve it."
            )
        ...
        result = self.prompt(
            {
                "kind": "path",
                "summary": f"repoterm wants {intent.replace('_', ' ')} access outside the current cwd",
                "details": [
    ...
    def ensure_command(
        ...
        if self.prompt is None:
            log_permission_check("run_command", signature, granted=False)
            raise RuntimeError(f"Command requires approval: {signature}. Start repoterm in TTY mode to approve it.")
        # Distinguish forced prompts (external trigger) from dangerous commands
        summary = (
            "repoterm wants to run a dangerous command"
            if not force_prompt_reason
            else "repoterm wants approval for this command"
        )
        result = self.prompt(
            {
                "kind": "command",
    ...
    def ensure_edit(self, target_path: str, diff_preview: str) -> None:
        ...
        if self.prompt is None:
            log_permission_check("edit_file", normalized_target, granted=False)
            raise RuntimeError(f"Edit requires approval: {normalized_target}. Start repoterm in TTY mode to review it.")
        result = self.prompt(
            {
                "kind": "edit",
                "summary": "repoterm wants to apply a file modification",
