"""Logging configuration for RepoTerm.

Provides structured logging with:
- 分级日志（DEBUG/INFO/WARNING/ERROR）
- 控制台和文件输出
- 日志轮转（按大小，防止无限增长）——这是当前唯一的正式轮转策略
- 结构化 JSON 日志（可选，便于机器解析；见 --structured-logs / REPOTERM_LOG_STRUCTURED）
...
# 日志文件路径
LOG_FILE = REPOTERM_DIR / "repoterm.log"
...
def setup_logging(
    ...
    # 创建根 logger
    root_logger = logging.getLogger("repoterm")
...
def get_logger(name: str) -> logging.Logger:
    ...
    Returns:
        配置好的子 logger
    """
    return logging.getLogger(f"repoterm.{name}")
...
def structured_logging_requested(cli_flag: bool = False) -> bool:
    ...
    ``REPOTERM_LOG_STRUCTURED`` 取真值（``1``/``true``/``yes``/``on``）。
    统一在入口处调用，避免各模块各自判断。
    """
    if cli_flag:
        return True
    return os.getenv("REPOTERM_LOG_STRUCTURED", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
