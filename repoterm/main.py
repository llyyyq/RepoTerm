...
from repoterm.cli_commands import try_handle_local_command
from repoterm.config import load_runtime_config
from repoterm.history import load_history_entries, save_history_entries
from repoterm.local_tool_shortcuts import parse_local_tool_shortcut
from repoterm.manage_cli import maybe_handle_management_command
...
from repoterm.tui.transcript import format_transcript_text
from repoterm.tui.types import TranscriptEntry
from repoterm.tty_app import run_tty_app
...
def _render_banner(runtime: dict | None, cwd: str, permission_summary: list[str], counts: dict[str, int]) -> str:
    ...
    lines = [
        "╔══════════════════════════════════════════════════════════╗",
        "║  🤖 RepoTerm - Your Terminal Coding Assistant    ║",
        "╠══════════════════════════════════════════════════════════╣",
        f"║  Model: {model:<46} ║",
        f"║  CWD: {cwd:<50} ║",
    ]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="RepoTerm - A lightweight terminal coding assistant",
        add_help=True,
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument(
        "--structured-logs",
        action="store_true",
        help="Emit JSON structured logs (also enabled via REPOTERM_LOG_STRUCTURED=true)",
    )
    parser.add_argument(
        "--trust-project-mcp",
        action="store_true",
        help="Load project-level .mcp.json (disabled by default for security; also via REPOTERM_TRUST_PROJECT_MCP=1)",
    )
    ...
    # Initialize logging
    from repoterm.logging_config import setup_logging, structured_logging_requested
    setup_logging(
    ...
    if args.install:
        from repoterm.install import main as install_main
        install_main()
        return
    ...
    try:
        ...
        print(
            ...
            "  3. Or edit ~/.repoterm/settings.json:\n"
    ...
    # 显示快速入门指南
    if not sys.stdin.isatty() or os.environ.get("REPOTERM_SHOW_GUIDE", "1") == "1":
