...
README_DOCUMENTED_GATE_NAMES: tuple[str, ...] = ()
...
def check_material_inventory(
    ...
    *,
    ...
    if not isinstance(current_app, dict):
        ...
        current_app = {}
    if current_app.get("logicalBoundary") != "product/app/repoterm_frontline":
        findings.append(
            _material_inventory_finding(
                "currentProductApp logicalBoundary is not product/app/repoterm_frontline",
                rule_id="MaterialInventoryProductApp",
            )
        )
    if current_app.get("currentSourceRoot") != "repoterm":
        findings.append(
            _material_inventory_finding(
                "currentProductApp currentSourceRoot is not repoterm",
                rule_id="MaterialInventoryProductApp",
            )
        )
