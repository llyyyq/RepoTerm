...
from repoterm.evidence_safety import redact_sensitive_payload, redact_sensitive_text
from repoterm.readiness_support import (
        ...
        previews.append(
            {
                ...
                "apply_notes": [
                    ...
                    "Run repoterm-readiness --json --fail-on blocked after applying.",
                ],
            }
        )
    return {
...
def _format_doctor_report(report, *, cwd: str) -> str:
    ...
    lines.extend(
        [
            ...
            "- It does not modify RepoTerm settings.",
        ]
    )
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="repoterm-readiness",
        description="Inspect RepoTerm provider/runtime readiness.",
    )
    parser.add_argument(
        "--cwd",
        default=".",
