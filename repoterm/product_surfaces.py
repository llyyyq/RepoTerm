...
from repoterm.background_tasks import get_slot_stats, list_background_tasks
from repoterm.config import (
    REPOTERM_EXTENSIONS_DIR,
    REPOTERM_MANAGED_POLICY_PATH,
    REPOTERM_SETTINGS_PATH,
    REPOTERM_USER_PROFILE_PATH,
...
from repoterm.hooks import get_hook_manager
...
def collect_instruction_layers(cwd: str | Path) -> list[InstructionLayer]:
    ...
    candidates = [
        ...
        ("global-user", "global", "user", REPOTERM_USER_PROFILE_PATH),
        ("global-managed", "global", "managed", REPOTERM_MANAGED_POLICY_PATH),
...
def extension_search_roots(cwd: str | Path) -> list[tuple[str, Path]]:
    return [
        ("global", REPOTERM_EXTENSIONS_DIR),
...
def _readiness_next_actions(
    *,
    ...
    if provider_ready and fallback_ready:
        actions.append("Keep fallback coverage in local runtime readiness checks.")
...
def _readiness_fallback_config_examples(
    *,
    ...
    if fallback_ready:
        return []
    settings_path = str(REPOTERM_SETTINGS_PATH)
        ...
        plan.append(
            {
                ...
                "status": "ready",
                "action": "Keep fallback coverage in local runtime readiness checks.",
                "command": "repoterm-readiness --json --fail-on blocked",
                "safety": "read-only",
            }
        )
        ...
        plan.append(
            {
                ...
                "status": "manual",
                "action": "Add explicit fallbackModels and matching provider credentials to RepoTerm settings.",
                "risk_scope": risk_scope,
    ...
    *,
    ...
    checks = [
        ...
        },
        {
            ...
            "action": (
                "Keep fallback coverage in local runtime readiness checks."
                if fallback_ready
        ...
        },
        {
            ...
            "action": "Add fallbackModels for deterministic runtime failover behavior.",
        },
        {
        ...
        },
        {
            ...
            "action": (
                "Run python benchmarks/llm_e2e_eval.py --task repository_retrieval "
                "--runs 1 --confirm-live for a real-provider smoke."
            ),
        },
    ]
