...
from repoterm.config import REPOTERM_DIR, REPOTERM_HISTORY_PATH
...
def load_history_entries() -> list[str]:
    ...
    if not REPOTERM_HISTORY_PATH.exists():
        _history_cache = (now, [])
        return []
    try:
        parsed = json.loads(REPOTERM_HISTORY_PATH.read_text(encoding="utf-8"))
...
def save_history_entries(entries: list[str]) -> None:
    ...
    REPOTERM_HISTORY_PATH.write_text(
