...
def try_handle_local_command(
    ...
    if user_input == "/state":
        try:
            from repoterm.state import handle_state_command
    ...
    if user_input == "/context":
        ...
        try:
            from repoterm.context_manager import load_context_state
    ...
    if user_input == "/model":
        try:
            runtime = load_runtime_config()
            from repoterm.model_registry import format_model_status
    ...
    if user_input.startswith("/model "):
        ...
        if not arg:
            from repoterm.model_registry import format_model_list
        ...
        from repoterm.model_registry import Provider, format_model_list
        for p in Provider:
...
def format_cybernetics_status() -> str:
    ...
    from repoterm.cybernetic_supervisor import CyberneticSupervisor, load_supervisor_report
    from repoterm.context_manager import load_context_state
