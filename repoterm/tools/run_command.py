...
def _command_output_encoding() -> str:
    ...
    Override with the ``REPOTERM_COMMAND_ENCODING`` env var — e.g. set it to
    ...
    """
    return os.environ.get("REPOTERM_COMMAND_ENCODING", "utf-8").strip() or "utf-8"
...
def _decode_command_output(data: bytes | str | None) -> str:
    ...
    UTF-8 with replacement chars so a bad ``REPOTERM_COMMAND_ENCODING`` value
