"""Tests for run_command output encoding (REPOTERM_COMMAND_ENCODING).
...
fix is an explicit override via REPOTERM_COMMAND_ENCODING.
"""
...
def _reload(monkeypatch):
    import repoterm.tools.run_command as rc
    return importlib.reload(rc)


def test_default_encoding_is_utf8(monkeypatch) -> None:
    monkeypatch.delenv("REPOTERM_COMMAND_ENCODING", raising=False)
...
def test_cp936_override_decodes_gbk(monkeypatch) -> None:
    ...
    monkeypatch.setenv("REPOTERM_COMMAND_ENCODING", "cp936")
    rc = _reload(monkeypatch)
...
def test_gbk_alias_works(monkeypatch) -> None:
    monkeypatch.setenv("REPOTERM_COMMAND_ENCODING", "gbk")
    rc = _reload(monkeypatch)
    assert rc._decode_command_output("目录".encode("gbk")) == "目录"


def test_bad_encoding_name_falls_back_to_utf8(monkeypatch) -> None:
    """An invalid REPOTERM_COMMAND_ENCODING must not crash execution."""
    monkeypatch.setenv("REPOTERM_COMMAND_ENCODING", "not-a-real-codec")
    rc = _reload(monkeypatch)
...
def test_blank_override_falls_back_to_utf8(monkeypatch) -> None:
    monkeypatch.setenv("REPOTERM_COMMAND_ENCODING", "   ")
    rc = _reload(monkeypatch)
    assert rc._command_output_encoding() == "utf-8"
