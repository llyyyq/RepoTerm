...
- structured_logging_requested honors CLI flag + REPOTERM_LOG_STRUCTURED env
...
def test_structured_formatter_emits_json_with_extras() -> None:
    ...
    assert parsed["module"] == "repoterm.tools"
...
def test_structured_logging_requested_cli_flag(monkeypatch) -> None:
    monkeypatch.delenv("REPOTERM_LOG_STRUCTURED", raising=False)
...
def test_structured_logging_requested_env(monkeypatch) -> None:
    for val in ("true", "1", "yes", "ON"):
        monkeypatch.setenv("REPOTERM_LOG_STRUCTURED", val)
...
def test_tool_execute_logs_success(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG, logger="repoterm.tools"):
...
def test_tool_execute_logs_crash(caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="repoterm.tools"):
...
def test_log_permission_check_emits(caplog) -> None:
    with caplog.at_level(logging.DEBUG, logger="repoterm.permissions"):
...
def test_log_session_event_emits(caplog) -> None:
    with caplog.at_level(logging.INFO, logger="repoterm.session"):
